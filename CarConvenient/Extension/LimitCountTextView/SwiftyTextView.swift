...
open class SwiftyTextView: UITextView {
    ...
    @IBInspectable open var placeholderColor1: UIColor = UIColor.lightGray { // placeholder color
        didSet {
            setNeedsLayout()
        }
    }
    ...
    override open func layoutSubviews() {
        ...
        if textLayer == nil {
            ...
            textLayer.foregroundColor = placeholderColor1.cgColor
            textLayer.font = font
        ...
        if showTextCountView {
            ...
            if countdownTextLayer == nil {
                ...
                countdownTextLayer.foregroundColor = placeholderColor1.cgColor
...
extension SwiftyTextView: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        let height = (text as NSString).boundingRect(with: CGSize(width: bounds.size.width, height: CGFloat.greatestFiniteMagnitude), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font : textView.font], context: nil).height
        if height > countdownTextLayer.frame.minY {
            countdownTextLayer.frame.origin = CGPoint(x: countdownTextLayer.frame.origin.x, y: height+5)
            contentOffset.y += countdownTextLayer.frame.height
        }else {
            countdownTextLayer.frame.origin = CGPoint(x: countdownTextLayer.frame.origin.x, y: bounds.size.height - countdownTextLayer.bounds.size.height + contentOffset.y)
        }
